...
void insertion_sort(int *arr,
                    int size) { // a more concise implementation can be done
  std::cout << "Sorting (Insertion sort)" << std::endl;
  int j = 0, hold = 0;
  for (int i = 1; i < size; ++i) { // second element first
    hold = arr[i];                 // checks an element, holds it
    for (j = i - 1; j >= 0 && arr[j] > hold;
         --j) {            // looks back, checks if the held element is greater
      arr[j + 1] = arr[j]; // if it is, pushes everything to the left of held
                           // element to the right
    }
    arr[j + 1] = hold; // takes j+1 (j can and will be -1 in case of a least
                       // element of the array) and equals it to hold, inserting
                       // hold into the position j stopped comparing at
  }
}
...
void merge(int *arr, int start, int mid, int end) {
  ...
  memcpy(&arr[start], arr1,
         (end - start) * sizeof(int)); // not necessarily start = arr1[0]
}
...
int main() {
  int arr[] = {3, 2, 5, 9, 8, 4, 7, 1, 6, 6, 3,
               2, 7, 5, 3, 7, 2, 1, 2, 3, 7, 5};
